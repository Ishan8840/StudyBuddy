...
import { Clock, Zap, Hand, Eye, Coffee, AlertCircle } from "lucide-react";
...
export const SessionProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (isSessionActive && !isOnBreak) {
      intervalRef.current = setInterval(() => {
        const now = new Date();
        const totalBreakTime = session.breaks.reduce((acc, b) => {
          if (!b[1]) return acc; // ongoing break not counted yet
          return acc + (new Date(b[1]) - new Date(b[0]));
        }, 0);
  ...
  const getFocusPercentage = (session) => {
    ...
    const now = session.timeEnded ? new Date(session.timeEnded) : new Date();
    ...
    return Math.max(0, Math.round((focusTime / totalSessionTime) * 100));
  };
